 ...
 *  - Finding all two sums takes N(N+1)/2 => O(N^2) where N is a number of numbers
 *  - Sorting all two sums takes O(N^2logN) because there are N(N+1)/2 two sums in array
 *  - Finding sums of two sums takes (N^2)log(N^2) => O((N^2)logN) where N is a number of numbers
 * 
 * Space complexity: O(N^2) where N is a number of numbers because we store all two sums
 */
